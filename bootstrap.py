...
One-liner examples:
  ...
  macOS/Linux:
    python3 -c "import urllib.request;urllib.request.urlretrieve('https://raw.githubusercontent.com/nubity/ai-frwk-setup/main/bootstrap.py','/tmp/bootstrap.py')" && python3 /tmp/bootstrap.py 1150636000118094005

  Windows (CMD):
    python -c "import urllib.request,sys;urllib.request.urlretrieve('https://raw.githubusercontent.com/nubity/ai-frwk-setup/main/bootstrap.py',sys.argv[1])" "%TEMP%\bootstrap.py" && python "%TEMP%\bootstrap.py" 1150636000118094005
...
def _pause_on_exit() -> None:
    """Keep the terminal window open when launched via OS UI."""
    if _is_double_click():
        print()
...
def _resolve_base_dir(explicit: str | None = None) -> Path:
    ...
    Priority:
      ...
      3. Platform default: ~/nubity-projects
    """
    if explicit:
...
def _can_ssh_to_github() -> bool:
    """Check if SSH authentication to GitHub succeeds."""
    try:
        result = subprocess.run(
...
def _gather_interactive() -> dict:
    ...
    print()
    print('  AI Framework - Workspace Setup')
    print('  ' + '=' * 30)
    print()

    crm_id = _prompt('CRM ID (19 digits)', validator=_validate_crm_id)

    base_dir_default = str(_resolve_base_dir())
    base_dir = _prompt('Base directory', default=base_dir_default)

    branch = _prompt('Framework branch', default='main')

    repository = ''
    if _prompt_yes_no('Target a specific repository?', default=False):
        repository = _prompt('Repository name')

    onedrive_path = ''
    if _prompt_yes_no('Provide explicit OneDrive path?', default=False):
        onedrive_path = _prompt('OneDrive path')
...
def main() -> None:
    ...
    if workspace_dir.is_dir():
        _error(f'Workspace directory already exists: {workspace_dir}')
        _info('If this is intentional, cd into it and run setup-workspace.py directly.')
        sys.exit(1)
